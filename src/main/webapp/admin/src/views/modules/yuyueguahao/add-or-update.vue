	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="挂号类型" prop="guahaoleixing">
					<el-select :disabled="ro.guahaoleixing" @change="guahaoleixingChange" v-model="ruleForm.guahaoleixing" placeholder="请选择挂号类型">
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.guahaoleixing" label="挂号类型" prop="guahaoleixing">
					<el-input v-model="ruleForm.guahaoleixing" placeholder="挂号类型" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="病情描述" prop="bingqingmiaoshu">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.bingqingmiaoshu" label="病情描述" prop="bingqingmiaoshu">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.bingqingmiaoshu"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/guahaoleixing/guahaoleixing`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.guahaoleixingOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
			...
			guahaoleixingChange () {
				this.$http({
					url: `follow/guahaoleixing/guahaoleixing?columnValue=`+ this.ruleForm.guahaoleixing,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.guahaofeiyong){
							this.ruleForm.guahaofeiyong = data.data.guahaofeiyong
						}
					} else {
						this.$message.error(data.msg);
					}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.bingqingmiaoshu = this.ruleForm.bingqingmiaoshu.replace(reg,'../../../ssmdki8hi7o/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
